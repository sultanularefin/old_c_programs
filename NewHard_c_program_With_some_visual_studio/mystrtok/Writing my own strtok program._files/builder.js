...
break;}this.setPosition(G,E);},autoCenter:function(){this.invitation.config.Events.beforeRenderUpdate();
D=Math.round(H.getPageWidth()/2)-Math.round(C/2);switch(this.invitation.config.verticalAlignment){case 0:J=this.invitation.config.verticalMargin;
break;case 1:J=Math.round(H.getPageHeight()/2)-Math.round(A/2);break;case 2:var N;
if(parseInt(navigator.appVersion)>3){if(navigator.appName=="Netscape"){N=window.innerHeight;
}if(navigator.appName.indexOf("Microsoft")!=-1){N=document.body.offsetHeight;}}J=N-this.invitation.config.height-this.invitation.config.verticalMargin;
break;}var S=5;var L=G;var M=E;var O=H.getPageLeftOffset();var R=H.getPageTopOffset();
var Q=Math.abs(O+D-L);var T=Math.abs(R+J-M);var P=Math.sqrt(Q*Q+T*T);var K=Math.round(P/50);
if(O+D>L){L=L+S+K;}if(O+D<L){L=L-S-K;}if(R+J>M){M=M+S+K;}if(R+J<M){M=M-S-K;}if(this.invitation.config.isHideBlockingElements){this.hideBlockingElements(true);
...
if(K){for(L=K.length-1;L>=0;L--){K[L].style.visibility="visible";K.pop();}}},destroy:function(){this.invitation.config.Events.beforeDestroy();
B();if(this.invitation.config.isHideBlockingElements){this.showBlockingElements();
}var K=this.invitation.container;K.style.visibility="hidden";function L(P){var O=P.attributes,N,Q,M;
if(O){Q=O.length;for(N=0;N<Q;N+=1){if(O[N]!=null){M=O[N].name;}if(typeof P[M]==="function"){P[M]=null;
...
if(!this.config.viewUrl){return;}var A=this.config.viewParams||"";if(this.config.viewUrl.indexOf("b.scorecardresearch.com")==-1){A+="&location="+escape(COMSCORE.SiteRecruit.Utils.location)+"&referrer="+escape(document.referrer)+"&site="+COMSCORE.SiteRecruit.Builder.invitation.config.acceptParams.siteCode;
}else{A+="&c1=12";A+="&c2="+COMSCORE.SiteRecruit.Builder.invitation.config.projectId;
A+="&c3="+COMSCORE.SiteRecruit.Builder.invitation.config.acceptParams.siteCode;A+="&c4=1";
A+="&c7="+escape(document.referrer);A+="&c8="+escape(COMSCORE.SiteRecruit.Utils.location);
A+="&c9="+escape(document.title);}var C=B.appendQueryParams(this.config.viewUrl,A);
B.fireBeacon(C);},updateContentPaths:function(){var B=COMSCORE.SiteRecruit.executingPath;
...
var A=new RegExp(B.declineHandlerTag,"g");this.config.Events.beforeRender();this.content=this.config.content.replace(C,B.acceptHandler);
...
this.container=document.createElement("div");var B=this.container;if(typeof(this.content)=="string"){if(this.config.silverlightConfig){this.replaceSilverlightParams();
}B.innerHTML=this.content;}else{if(this.config.silverlightConfig){this.replaceSilverlightParams();
}B.appendChild(this.content);}var A=B.style;A.position="absolute";A.left="0px";A.top="0px";
A.zIndex=10001;A.visibility="hidden";document.body.appendChild(B);this.config.revealDelay=this.config.revealDelay||1;
var C=this;setTimeout(function(){C.inject();},this.config.revealDelay);this.config.Events.afterRender();
},removeInvitation:function(B){B=B||1;var A=this;setTimeout(function(){COMSCORE.SiteRecruit.Invitation.Renderer.destroy();
},B);},decline:function(){this.config.Events.beforeDecline();this.removeInvitation(COMSCORE.SiteRecruit.Invitation.REMOVE_DELAY.DECLINE);
return false;},accept:function(){var B=COMSCORE.SiteRecruit.Utils;this.config.Events.beforeAccept();
this.removeInvitation(COMSCORE.SiteRecruit.Invitation.REMOVE_DELAY.ACCEPT);this.surveyData=COMSCORE.SiteRecruit.Builder.collectSurveyData();
var D=COMSCORE.SiteRecruit.Builder.invitation.config;if(this.config.viewUrl.indexOf("b.scorecardresearch.com")==-1){var A="?log=comscore/accept/"+D.projectId+"-accept.log&location="+escape(COMSCORE.SiteRecruit.Utils.location)+"&referrer="+escape(document.referrer)+"&site="+D.acceptParams.siteCode;
}else{var A="c1=12";A+="&c2="+D.projectId;A+="&c3="+D.acceptParams.siteCode;A+="&c4=2";
A+="&c7="+escape(document.referrer);A+="&c8="+escape(COMSCORE.SiteRecruit.Utils.location);
A+="&c9="+escape(document.title);A+="&c10=comscore/accept/"+D.projectId+"-accept.log";
...
}}return B.appendQueryParams(C,A);},replaceSilverlightParams:function(){var A="";
var B=this.config.silverlightConfig;objects=document.getElementsByTagName("param");
A="background="+B.bgImage;A=A+",closeBtn="+B.closeBtnImage;A=A+",yesBtn="+B.yesBtnImage;
A=A+",noBtn="+B.noBtnImage;A=A+",closeX="+B.closeXCoord;A=A+",closeY="+B.closeYCoord;
A=A+",yesX="+B.yesXCoord;A=A+",yesY="+B.yesYCoord;A=A+",noX="+B.noXCoord;A=A+",noY="+B.noYCoord;
A=A+",acceptFunc="+B.acceptFunc;A=A+",declineFunc="+B.declineFunc;A='<param name="initParams" value="'+A+'"/>';
this.content=this.content.replace(/<param name="initParams" value=[^<]*\/>/,A);this.config.content=this.content;
}};COMSCORE.SiteRecruit.Invitation.Standard=function(A){COMSCORE.SiteRecruit.Invitation.Standard.superclass.constructor.call(this,A);
...
COMSCORE.SiteRecruit.Utils.UserPersistence.setUserObj(A);if(document.location.protocol=="https:"){var B=this.appendSurveyDataToAcceptUrl(this.config.secureUrl);
}else{var B=this.appendSurveyDataToAcceptUrl(this.config.acceptUrl);}if(COMSCORE.SiteRecruit.Silverlight){COMSCORE.SiteRecruit.Silverlight.acceptUrl=B;
...
A.statename=COMSCORE.SiteRecruit.CONSTANTS.STATE_NAME.IDLE;A.pid=this.config.projectId;
COMSCORE.SiteRecruit.Utils.UserPersistence.setUserObj(A);if(document.location.protocol=="https:"){var B=this.appendSurveyDataToAcceptUrl(this.config.secureUrl);
}else{var B=this.appendSurveyDataToAcceptUrl(this.config.acceptUrl);}COMSCORE.SiteRecruit.Utils.fireBeacon(B);
return true;},toString:function(){return"Email";}});COMSCORE.SiteRecruit.Invitation.DD=function(A){COMSCORE.SiteRecruit.Invitation.DD.superclass.constructor.call(this,A);
...
var D=this.config.trackerWindow;if(document.location.protocol=="https:"){A("surveyUrl",this.config.secureUrl);
}else{A("surveyUrl",this.config.acceptUrl);}A("cookieName",C.name);A("cookiePath",C.path);
A("cookieDomain",C.domain);A("cookieDuration",C.duration);A("cookieDate",C.date);
A("trackerPageConfigUrl",COMSCORE.SiteRecruit.executingPath+this.config.trackerWindow.trackerPageConfigUrl);
COMSCORE.SiteRecruit.DDKeepAlive.setDDTrackerCookie();var B="";if(document.location.protocol=="https"){B=this.appendSurveyDataToAcceptUrl(this.config.secureUrl);
}else{B=this.appendSurveyDataToAcceptUrl(this.config.url);}COMSCORE.SiteRecruit.Broker.processDDInProgress();
if(COMSCORE.SiteRecruit.Silverlight){var E=D.features||"resizable=1,status=1,scrollbars=1";
...
var A=C.Broker;return{version:"5.0.3",init:function(){if(C.version!==this.version){return;
...
var D={};D.cookiename=A.config.cookie.name;D.cookieoptions=A.config.cookie;D.pid=this.config.invitation[0].projectId;
B.UserPersistence.setUserObj(D);var F=B.UserPersistence.getUserObj(D);return(F&&F.state.name==C.CONSTANTS.STATE_NAME.IDLE);

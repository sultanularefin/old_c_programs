...
if (!MS.Support.Fms.Gsfx.GsfxSurvey)
{
	MS.Support.Fms.Gsfx.GsfxSurvey = function (survey)
	{
			...
			if (survey.isStandalone)
			{
				...
				if ((!survey.isInvitation) && (!survey.isShowAll) && isOnUnloadSurvey)
				{
					...
					if (window.opener != null
						&& (window.history.length == 0 || (window.navigator.userAgent.indexOf("MSIE") < 0 && window.history.length == 1))
		...
		function showHiddenSurvey()
		{
			...
			if (isOnUnloadSurvey && window.opener && (window.history.length == 0 || (window.navigator.userAgent.indexOf("MSIE") < 0 && window.history.length == 1)))
			{

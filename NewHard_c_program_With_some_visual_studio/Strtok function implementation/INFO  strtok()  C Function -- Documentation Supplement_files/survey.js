	...
	MS.Support.Fms.SurveyQuestionOperator =
	{
		...
		ghrOperator: function (question)
		{
			...
			this.validateRequired = function ()
			{
				...
				if (isRanking && count == Math.min(visibleRowCount, visibleColumnCount)
                || !isRanking && count == visibleRowCount)
				{
					return true;
				}
			...
			this.prepare = function ()
			{
				...
				for (var index = 0; index < ghrRows.length; ++index)
				{
					...
					var showIf = true, SelectIf = false;
			...
			this.prepare = function ()
			{
				...
				for (var index = 0; index < allOptions.length; ++index)
				{
					...
					if (bShow)
					{
						...
						{
							if (question.getQuestionType() == "CHOICE-LIST")
							{
								var hasNoSelectedOption = true;
								for (var i = 0; i < allOptions.length; i++)
								{
									if (allOptions[i].option.selected && i != index)
									{
										hasNoSelectedOption = false;
										break;
									}
								}
								selectObject.options[0].selected = hasNoSelectedOption ? true : false;
							}
